...
std::vector<AlignedPair> adaptive_banded_simple_event_align_C(SquiggleRead& read, const PoreModel& pore_model, char* sequence,int& outSize)
{
  /*
  ...
  */
    ...
    for(size_t i = 0; i < n_kmers; ++i) {
        ...
        kmer_ranks[i] = alphabet->kmer_rank(substring, k);
        //<<<<<<<<< New replacement over
    }
...
*/
    ...
    */
    ...
    while(curr_kmer_idx >= 0 && curr_event_idx >= 0) {
        ...
        size_t kmer_rank = alphabet->kmer_rank(sequence, k);
        //<<<<<<<<<<<<<New Replacement over
...
std::vector<AlignedPair> adaptive_banded_simple_event_align_CPP(SquiggleRead& read, const PoreModel& pore_model, const std::string& sequence)
{
    /*
      This is the original function in nanopolish
...
std::vector<AlignedPair> adaptive_banded_simple_event_align(SquiggleRead& read, const PoreModel& pore_model, const std::string& sequence){
  char* charSeq =(char*)malloc(sizeof(char)*(sequence.size()+1));

  int i;
  for(i=0;i<sequence.size();i++){
    charSeq[i]=sequence[i];
  }
  charSeq[i]='\0';
  //printf("Done changing data structures from cpp to C");

  int outSize=0;
  std::vector<AlignedPair> cFunctionReturn= adaptive_banded_simple_event_align_C(read,pore_model,charSeq,outSize);

  /*std::vector<AlignedPair> cppFunctionReturn;

  while(i<outSize){
    cppFunctionReturn.push_back({cFunctionReturn[i].ref_pos,cFunctionReturn[i].read_pos});
    i++;
  }
  free(cFunctionReturn);*/
  free(charSeq);
